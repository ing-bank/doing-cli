...
def cmd_list(team: str, area: str, iteration: str, organization: str, project: str):
    ...
    query = "SELECT [System.Id],[System.Title],[System.AssignedTo],[System.WorkItemType] FROM WorkItems WHERE ([System.State] = 'Active' OR [System.State] = 'New') "
    ...
    with Live() as live:
        ...
        query = f"az repos pr list --repository {repo_name} --organization {organization} --project {project} "
        query += f"--status 'active' --query '[].pullRequestId'"
        ...
        for pr_id in track(
            active_pullrequest_ids, description="Processing pull requests"
        ):
            linked_workitems = run_command(
                f"az repos pr work-item list --id {pr_id} --query '[].id'"
            )
...
def build_table(work_items, workitem_prs, iteration, last_build=False) -> Table:
    ...
    for item in work_items:
        ...
        # relations = run_command(f"az boards work-item show --id {item_id} --query \"relations[?attributes.name=='Pull Request'].url\"") # example id 99035
        # item_linked_prs = []
        ...
        table.add_row(
            str(item_id), item_title, item_createdby, item_type, item_linked_prs
        )
