...
def cmd_create_pr(
    ...
    work_item = run_command(
        f"az boards work-item show --id {work_item_id} --query 'fields'"
    )
    work_item_title = work_item.get("System.Title")

    # Info on remote branches
    remote_branches = run_command(
        f"az repos ref list --repository {repo_name} --query '[].name'"
    )
    remote_branches = [
    ...
    if branch_name in remote_branches:
        console.print(
            f"> Branch '[cyan]{branch_name}[/cyan]' already exists on remote, using that one"
        )
        # Check if there is not already an existing PR for this branch
        prs = run_command(f"az repos pr list -r {repo_name} -s {branch_name}")
        if len(prs) >= 1:
            console.print(
                f"> Pull request {prs[0].get('pullRequestId')} already exists for branch '[cyan]{branch_name}[/cyan]', aborting."
            )
            return None
    else:
        branch = run_command(
            f"az repos ref create --name 'heads/{branch_name}' --repository '{repo_name}' --object-id '{master_branch_object_id}' --project '{project}' --organization '{organization}'"
        )
