...
from doing.utils import run_command, get_repo_name, to_snake_case, get_az_devop_user_email

from rich.console import Console
console = Console()

def cmd_create_pr(work_item_id: str, draft: bool, auto_complete:bool, self_approve: bool, reviewers:str, team: str, area: str, iteration: str, organization: str, project: str):
    """
    API doc:
    ...
    master_branch_object_id = run_command(f"az repos ref list --repository '{repo_name}' --query \"[?name=='refs/heads/master'].objectId\"")[0]
    ...
    if branch_name in remote_branches:
        ...
        if len(prs) >= 1:
           console.print(f"> Pull request {prs[0].get('pullRequestId')} already exists for branch '[cyan]{branch_name}[/cyan]', aborting.")
           return None 
    else:
        branch = run_command(f"az repos ref create --name 'heads/{branch_name}' --repository '{repo_name}' --object-id '{master_branch_object_id}' --project '{project}' --organization '{organization}'")
    ...
    command += f"--transition-work-items 'true' "
    command += f"--delete-source-branch 'true' "
    ...
    if draft:
        console.print(f"\t[red]>[/red] marked PR as draft'")
    if auto_complete:
        console.print(f"\t[red]>[/red] Set auto-complete to True'")
    if self_approve:
